...
const HotelsPage = () => {
  ...
  const handleAddHotel = async (e) => {
    ...
    formData.append('phone', newHotel.phone);
    formData.append('price', newHotel.price);
    formData.append('currency', newHotel.currency);
    ...
    try {
      // Envoi à Django
      await addHotelMutation(formData).unwrap();

      // Petite pause pour laisser Cloudinary traiter l'image
      setTimeout(() => {
        setShowModal(false);
  ...
  return (
    <DashboardLayout 
      ...
      subtitle={<>Hôtels <span className="text-gray-400 ml-2 font-light">{hotels?.length || 0}</span></>}
      actionButton={createButton}
    >
      
      {/* GRILLE D'AFFICHAGE */}
      {isLoading ? (
        <div className="flex justify-center p-10 text-gray-400 italic">Chargement des hôtels...</div>
      ) : (
          ...
          {hotels.map(hotel => (
            <div key={hotel.id} className="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 flex flex-col hover:shadow-md transition-all duration-300 group">
              <div className="h-44 w-full overflow-hidden bg-gray-100">
                <img 
  src={
    hotel.image // Si l'image existe
      ? hotel.image // On l'affiche telle quelle (car Django donne déjà l'URL complète)
      : "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=600" // Sinon la plage
  }
                 alt={hotel.name} 
  className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" 
  onError={(e) => {
    // Sécurité au cas où le lien Cloudinary est mort
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-4 border-b border-dashed border-gray-200 flex items-center gap-4 shrink-0">
              <button onClick={() => !isSubmitting && setShowModal(false)} className="text-gray-400 hover:text-black transition-colors p-1 text-2xl">←</button>
              <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest">Créer un nouveau hôtel</h3>
            </div>
            ...
            <form onSubmit={handleAddHotel} className="flex flex-col overflow-hidden">
                  ...
                  {['name', 'address', 'email', 'phone', 'price'].map(field => (
                    <div key={field} className="flex flex-col gap-2">
                      <label className="text-xs font-bold text-gray-700">
                        {field === 'name' ? "Nom de l'hôtel" : field.charAt(0).toUpperCase() + field.slice(1)}
                      </label>
                      <input 
                        type={field === 'price' ? 'number' : field === 'email' ? 'email' : 'text'} 
                        required 
                        value={newHotel[field]} 
                        onChange={e => setNewHotel({ ...newHotel, [field]: e.target.value })} 
                        className="border border-gray-200 bg-gray-50/50 rounded-xl p-3 text-sm outline-none focus:border-gray-400 transition-all" 
                        placeholder={`${field}...`} 
                      />
                    </div>
                  ))}
                ...
                <div className="mt-6">
                  ...
                  <div className="border-2 border-dashed border-gray-200 rounded-2xl h-40 flex flex-col items-center justify-center relative bg-gray-50/30 hover:bg-gray-50 transition-colors overflow-hidden group">
                    {previewImage ? (
                      ...
                      <div className="flex flex-col items-center gap-2 text-gray-300 group-hover:text-gray-400">
                        <FaCamera size={32} />
              ...
              <div className="p-6 border-t border-gray-100 flex justify-end shrink-0">
                <button 
                  ...
                  className={`${isSubmitting ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#444444] hover:bg-black'} text-white px-10 py-3 rounded-xl font-bold text-sm transition-all shadow-lg active:scale-95`}
                >
