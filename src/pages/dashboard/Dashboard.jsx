...
import DashboardLayout from '../../components/layout/DashboardLayout';
import { FaFileAlt, FaEnvelope, FaUsers, FaHotel, FaProjectDiagram } from 'react-icons/fa';
// On utilise ton service RTK Query qui est déjà connecté à Django
import { useGetHotelsQuery } from '../../services/hotelService';

const Dashboard = () => {
  // Récupération des hôtels de l'utilisateur en temps réel
  const { data: hotels = [] } = useGetHotelsQuery();

  const stats = [
    { label: 'Formulaires', value: '125', icon: <FaFileAlt />, color: 'bg-purple-600' },
    { label: 'Messages', value: '40', icon: <FaEnvelope />, color: 'bg-teal-500' },
    { label: 'Utilisateurs', value: '600', icon: <FaUsers />, color: 'bg-yellow-500' },
    { label: 'E-mails', value: '25', icon: <FaEnvelope />, color: 'bg-red-500' },
    // Cette valeur se mettra à jour toute seule quand tu créeras un hôtel
    { label: 'Hôtels', value: hotels.length, icon: <FaHotel />, color: 'bg-purple-800' },
    { label: 'Entités', value: '02', icon: <FaProjectDiagram />, color: 'bg-blue-600' },
  ];
    ...
    <DashboardLayout title="Dashboard">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {stats.map((stat, idx) => (
          <div 
            key={idx} 
            className="bg-white rounded-2xl p-6 shadow-sm flex items-center gap-6 border-none hover:shadow-md transition-all duration-300"
          >
            {/* Icône avec couleur dynamique */}
            <div className={`${stat.color} w-14 h-14 rounded-full flex items-center justify-center text-white text-2xl shrink-0`}>
              {stat.icon}
            </div>
            ...
            <div className="min-w-0">
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-lg text-gray-500 font-light truncate">
                  {stat.label}
                </span>
              </div>
              <p className="text-xs text-gray-400 mt-1">Données à jour</p>
            </div>
          </div>
        ))}
      </div>
